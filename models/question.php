...
class question_class extends AWS_MODEL
{
	...
	// Modify by wecenter
	public function get_questions_list($page = 1, $per_page = 10, $sort = null, $topic_id = null, $category_id = null, $answer_count = null, $day = 30, $is_recommend = false, $uids = null, $pull_index = false, $in_feature = false, $not_in_category = null)
	{
		...
		{
			// Modify by wecenter
			$question_info_list = $this->get_question_list_by_topic_ids($topic_id, $category_id, $answer_count, $order_key, $is_recommend, $page, $per_page, $uids, $pull_index, $in_feature, $not_in_category);
		}
		else
		{
			$where = array();
			...
			// Modify by wecenter			
			if (sizeof($uids) > 0 AND is_array($uids))
			{
				array_walk_recursive($uids, 'intval_string');
				
				$extra_query[] = "published_uid IN (" . implode(',', $uids) . ")";
			}
			...
			if ($pull_index)
			{
				$extra_query[] = "pull_index = 1";
			}
			
			if ($extra_query)
			{
				$extra_query_string = implode(' OR ', $extra_query);
				
				if ($where_query)
				{
					$extra_query_string = ' OR ' . $extra_query_string;
				}
			}
			
			if ($where OR $extra_query_string)
			{
				$where_query = 'add_time <= ' . time() . ' AND (' . $where_query . $extra_query_string . ')';
			}
			
			if ($where_query AND $not_in_category)
			{
				$where_query = '(' . $where_query . ') AND category_id NOT IN(' . implode(',', $not_in_category) . ')';
			}
			else if ($not_in_category)
			{
				$where_query = 'category_id NOT IN(' . implode(',', $not_in_category) . ')';
			}
	
			$question_info_list = $this->fetch_page('question', $where_query, $order_key, $page, $per_page);
	...
	public function get_hot_question($category_id = 0, $topic_id = null, $day = 30, $page, $per_page)
	{
		...
		if ($category_id)
		{
			...
			$question_all = $this->fetch_page('question', "add_time > " . $add_time . " AND add_time <= " . time() . " AND thanks_count > 0 AND category_id IN(" . implode(',', $this->model('system')->get_category_with_child_ids('question', $category_id)) . ')', 'thanks_count DESC', $page, $per_page);
	 ...
	 */
	public function save_question($question_content, $question_detail, $published_uid, $anonymous = 0, $ip_address = null, $buy_link = null, $add_time = null, $sub_title = null)	// Modify by wecentr
	{
		if (!$ip_address)
		{
			$ip_address = fetch_ip();
		...
		if ($question_id = $this->insert('question', array(
			...
			'buy_link' => htmlspecialchars($buy_link),	// Modify by wecentr
			'sub_title' => htmlspecialchars($sub_title),	// Modify by wecentr
	 ...
	 * @return boolean true|false
	 */
	public function update_question($question_id, $question_content, $question_detail, $uid, $verified = true, $modify_reason = '', $sub_title = null) // Modify by anwsion
	{		
		...
		if ($verified)
		{
			...
			$data['sub_title'] = $sub_title;	// Modify by anwsion
		...
		if ($question_lnk)
		{
			foreach ($question_lnk as $key => $question_content)
			{
				$question_lnk_list[] = array(
					...
					// Modify by wecenter
					'question_detail' => $question_info[$key]['question_detail']
				);
			}
		}
	...
	public function get_question_list_by_topic_ids($topic_ids, $category_id = null, $answer_count = null, $order_by = 'question_id DESC', $is_recommend = false, $page, $per_page, $uids = null, $pull_index = false, $in_feature = false, $not_in_category = null)	// Modify by wecenter
	{
		if (!$topic_ids)
		{
			return false;
		...
		if (sizeof($uids) > 0 AND is_array($uids))
		{
			...
			$extra_query[] = "question.published_uid IN (" . implode(',', $uids) . ")";
		}
		
		if ($pull_index)
		{
			$extra_query[] = "question.pull_index = 1";
		}
		...
		if ($extra_query)
		{
			...
			if ($where_query)
			{
				if ($in_feature)
				{
					$extra_query_string = ' AND (' . $extra_query_string .  ')';
				}
				else
				{
					$extra_query_string = ' OR ' . $extra_query_string;
		...
		if ($where_query OR $extra_query_string)
		{
			$where_query = 'question.add_time <= ' . time() . ' AND (' . $where_query . $extra_query_string . ')';
		}
		
		if ($where_query AND $not_in_category)
		{
			$where_query = '(' . $where_query . ') AND question.category_id NOT IN(' . implode(',', $not_in_category) . ')';
		}
		else if ($not_in_category)
		{
			$where_query = 'question.category_id NOT IN(' . implode(',', $not_in_category) . ')';
		}
		
		if (!$found_rows = AWS_APP::cache()->get($found_rows_cache_key))
		{
			$_found_rows = $this->query_row('SELECT COUNT(DISTINCT question.question_id) AS count FROM ' . $this->get_table('question') . ' AS question LEFT JOIN ' . $this->get_table('topic_question') . ' AS topic_question ON question.question_id = topic_question.question_id WHERE ' . $where_query);	// Modify by wecenter
	...
	// Modify by wecenter
	public function pull_index_question($question_id, $pull_index = true)
	{
		if ($pull_index)
		{
			if (!$quesion_info = $this->get_question_info_by_id($question_id))
			{
				return false;
			}
			
			$this->query("INSERT INTO `" . get_table('ios_push') . "` (`device_token`, `message`, `item_id`) SELECT `device_token`, '" . $this->quote($quesion_info['question_content']) . "', '" . $question_id . "' FROM `" . get_table('ios_device_token') . "`");
		}
	...
	public function update_buy_link_by_id($question_id, $buy_link)
	{
		return $this->update('question', array(
			'buy_link' => htmlspecialchars($buy_link)
		), 'question_id = ' . intval($question_id));
	}
	...
	public function get_questions_thanks($question_ids, $uid)
	{
		if (!is_array($question_ids) OR !$uid)
		{
			return false;
		}
		...
		if ($get_questions_thanks = $this->fetch_all('question_thanks', "question_id IN (" . implode(',', $question_ids) . ") AND uid = " . intval($uid)))
		{
			foreach ($get_questions_thanks AS $key => $val)
			{
				$result[$val['question_id']] = $val['id'];
			}
		}
	...
	// Modify by wecenter
	public function get_question_thanks_users_by_group_id($question_id, $group_id, $limit)
	{
		if (!$question_id OR !$group_id)
		{
			return false;
		}
		
		if ($data = AWS_APP::cache()->get('get_question_thanks_users_by_group_id_' . intval($question_id) . '_' . intval($group_id) . '_' . $limit))
		{
			return $data;
		}
		
		if ($question_thanks = $this->query_all("SELECT uid FROM " . get_table('question_thanks') . " WHERE question_id = " . intval($question_id) . " ORDER BY id DESC"))
		{
			foreach ($question_thanks AS $key => $val)
			{
				$uids[] = $val['uid'];
			}
			
			if ($user_infos = $this->fetch_all('users', 'uid IN (' . implode(',', $uids) . ') AND group_id = ' . intval($group_id), $limit))
			{
				 unset($uids);
				...
				if ($data)
				{
					AWS_APP::cache()->set('get_question_thanks_users_by_group_id_' . intval($question_id) . '_' . intval($group_id) . '_' . $limit, $data, get_setting('cache_level_low'));
				}
	...
	public function get_thanks_questions_by_uid($uid, $limit)
	{
		if (!$uid OR !$limit)
		{
			return false;
		...
		if ($question_thanks = $this->query_all("SELECT question_id FROM " . get_table('question_thanks') . " WHERE uid = " . intval($uid) . " ORDER BY id DESC LIMIT " . $limit))
		{
			foreach ($question_thanks AS $key => $val)
			{
				$question_ids[] = $val['question_id'];
			...
			return $this->fetch_all('question', 'question_id IN (' . implode(',', $question_ids) . ')');
		}
	}
	...
	public function question_thanks($question_id, $uid, $user_name)
	{
		...
		else
		{
			...
			// Modify by anwsion
			if (!$this->model('integral')->fetch_log($this->user_id, 'QUESTION_THANKS_' . gmdate('Ymd')))
			{
				$this->model('integral')->process($uid, 'QUESTION_THANKS_' . gmdate('Ymd'), get_setting('integral_system_config_thanks'), '感谢主题 #' . $question_id, $question_id);
			}
