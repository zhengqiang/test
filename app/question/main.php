...
class main extends AWS_CONTROLLER
{
	...
	public function index_action()
	{
		...
		// Modify by anwsion
		$question_info['user_info']['user_group'] = $this->model('account')->get_user_group($question_info['user_info']['group_id'], $this->model('account')->get_user_group_by_reputation($question_info['user_info']['reputation'], 'group_id'));
		...
		// Modify by wecenter
		$question_info['question_detail'] = $this->model('link_replace')->replace_content($question_info['question_detail']);
		...
		// Modify by wecenter
		$question_related_list = $this->model('question')->get_related_question_list($question_info['question_id'], $question_info['question_content'], 4);
		...
		// Modify by anwsion
		TPL::set_meta('description', $question_info['question_content'] . ' - 专注品质生活的购物分享社区');
		...
		// Modify by wecenter
		TPL::assign('question_thanks_users', $this->model('question')->get_question_thanks_users_by_group_id($question_info['question_id'], 100, 5));
