...
class ajax extends AWS_CONTROLLER
{
	public function get_access_rule()
	{
		...
		$rule_action['actions'] = array(
			...
			'channelsread_status'	// Modify by anwsion
		);
	...
	function profile_setting_action()
	{
		...
		// Modify by anwsion
		if ($this->user_info['permission']['custom_amazon_tag'])
		{
			$update_data['amazon_tag'] = htmlspecialchars($_POST['amazon_tag']);
		}
	...
	// Modify by anwsion
	public function channelsread_status_action()
	{
		$channelsread = array(
			1 => 'unread',
			2 => 'unread',
		...
		{
			if ($channelsread_status = unserialize(HTTP::get_cookie('channelsread_status')))
			{
				foreach ($channelsread AS $key => $val)
				{
					$channel_question = end($this->model('question')->get_questions_list(1, 1, null, null, $key));
					
					if ($channel_question['question_id'] AND $channel_question['question_id'] == $channelsread_status[$key]['question_id'])
					{
						$channelsread[$key] = 'read';
					}
				}
			}
		}
		
		echo json_encode($channelsread);
	}
	
	// Modify by anwsion
	public function coin_log_action()
	{
		if ($log = $this->model('coin')->fetch_all('coin_log', 'uid = ' . $this->user_id, 'time DESC', (intval($_GET['page']) * 50) . ', 50'))
		{
			foreach ($log AS $key => $val)
			{
				$parse_items[$val['id']] = array(
			...
			TPL::assign('log', $log);
			TPL::assign('log_detail', $this->model('coin')->parse_log_item($parse_items));
		}
		
		TPL::output('account/ajax/coin_log');
	}
}
